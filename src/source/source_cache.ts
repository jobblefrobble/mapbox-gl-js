...
class SourceCache extends Evented {
  ...
  onAdd(map: MapboxMap) {
    ...
    this._minTileCacheSize =
      this._minTileCacheSize === undefined && map
        ? map._minTileCacheSize
        : this._minTileCacheSize;
    this._maxTileCacheSize =
      this._maxTileCacheSize === undefined && map
        ? map._maxTileCacheSize
        : this._maxTileCacheSize;
  }
  ...
  loaded(): boolean {
    ...
    for (const t in this._tiles) {
      ...
      if (
        tile.state !== "errored" &&
        (tile.state !== "loaded" || !tile.bucketsLoaded())
      )
        return false;
    }
    return true;
    ...
    if (symbolLayer) {
      return renderables
        .sort((a_: Tile, b_: Tile) => {
          const a = a_.tileID;
          const b = b_.tileID;
          const rotatedA = new Point(a.canonical.x, a.canonical.y)._rotate(
            this.transform.angle
          );
          const rotatedB = new Point(b.canonical.x, b.canonical.y)._rotate(
            this.transform.angle
          );
          return (
            a.overscaledZ - b.overscaledZ ||
            rotatedB.y - rotatedA.y ||
  ...
  _tileLoaded(
    ...
    if (err) {
      ...
      // to use it as an overscaled tile instead of the missing one.
        ...
        const updateForTerrain =
          this._source.type === "raster-dem" && this.usedForTerrain;
  ...
  _getLoadedTile(tileID: OverscaledTileID): Tile | null | undefined {
    ...
    const cachedTile = this._cache.getByKey(
  ...
  updateCacheSize(transform: Transform, tileSize?: number) {
    ...
    const viewDependentMaxSize = Math.floor(
      approxTilesInView * commonZoomRange
    );
    const minSize =
      typeof this._minTileCacheSize === "number"
        ? Math.max(this._minTileCacheSize, viewDependentMaxSize)
        : viewDependentMaxSize;
    const maxSize =
      typeof this._maxTileCacheSize === "number"
        ? Math.min(this._maxTileCacheSize, minSize)
        : minSize;
  ...
  update(
    ...
    if (!this.used && !this.usedForTerrain) {
      ...
      idealTileIDs = transform
        .getVisibleUnwrappedCoordinates(this._source.tileID as CanonicalTileID)
        .map(
          (unwrapped) =>
      ...
      if (
        this._source.minzoom <= 1.0 &&
        transform.projection.name === "globe"
      ) {
    ...
    if (
      idealTileIDs.length > 0 &&
      this.castsShadows &&
      directionalLight &&
      this.transform.projection.name !== "globe" &&
      !this.usedForTerrain &&
      !isRasterType(this._source.type)
    ) {
      ...
      const shadowCasterTileIDs = transform.extendTileCoverForShadows(
        idealTileIDs,
        directionalLight,
        idealZoom
      );
    ...
    if (isRasterType(this._source.type) && idealTileIDs.length !== 0) {
      const parentsForFading: Partial<
        Record<string | number, OverscaledTileID>
      > = {};
      ...
      for (const id of ids) {
        ...
        const parentTile = this.findLoadedParent(
          tileID,
          Math.max(
            tileID.overscaledZ - SourceCache.maxOverzooming,
  ...
  _updateRetainedTiles(
  ...
  ): Partial<Record<number | string, OverscaledTileID>> {
    ...
    const minZoom = idealTileIDs.reduce(
      (min, id) => Math.min(min, id.overscaledZ),
      Infinity
    );
    const maxZoom = idealTileIDs[0].overscaledZ;
    assert(minZoom <= maxZoom);
    const minCoveringZoom = Math.max(
      maxZoom - SourceCache.maxOverzooming,
      this._source.minzoom
    );
    const maxCoveringZoom = Math.max(
      maxZoom + SourceCache.maxUnderzooming,
      this._source.minzoom
    );
  ...
  _addTile(tileID: OverscaledTileID): Tile {
    ...
    if (!cached) {
      ...
      this._loadTile(
        tile,
        this._tileLoaded.bind(this, tile, tileID.key, tile.state)
      );
    }
  ...
  tilesIn(
    ...
    for (const tileID in this._tiles) {
      ...
      for (const wrap of tilesToCheck) {
        const tileResult = queryGeometry.containsTile(
          tile,
          transform,
          use3DQuery,
          wrap
  ...
  _getRenderableCoordinates(
    ...
    const coords = this.getRenderableIds(symbolLayer, includeShadowCasters).map(
      (id) => this._tiles[id].tileID
    );
    const isGlobe = this.transform.projection.name === "globe";
    for (const coord of coords) {
      coord.projMatrix = this.transform.calculateProjMatrix(
        coord.toUnwrapped()
      );
      if (isGlobe) {
        coord.expandedProjMatrix = this.transform.calculateProjMatrix(
          coord.toUnwrapped(),
          false,
          true
        );
  ...
  sortCoordinatesByDistance(
    ...
    for (const id of sortedCoords) {
      ...
      precomputedDistances[id.key] =
        (centerX - camPos[0]) * camFwd[0] +
        (centerY - camPos[1]) * camFwd[1] -
        camPos[2] * camFwd[2];
    }
  ...
  _preloadTiles(
    ...
    const terrain = this.map.painter.terrain;
    const tileSize =
      this.usedForTerrain && terrain
        ? terrain.getScaledDemTileSize()
    ...
    asyncAll(
      ...
      (tileID, done) => {
        const tile = new Tile(
          tileID,
          this._source.tileSize * tileID.overscaleFactor(),
          this.transform.tileZoom,
          this.map.painter,
          this._isRaster
        );
